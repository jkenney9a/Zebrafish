...
def distance_from_center(df, frame, top, bottom, left, right):
    """
    Input: dataframe of tracking data, frame to be analyzed, coordinates for 
    the center of the tank
    
    Output: absolute value of the distance the object is from the center point 
    of the tank
    """
    #Get x,y coordinates of center:
    center = (np.mean([left, right]), np.mean([top, bottom]))
    
    x = center[0] - df['x'][frame]
    y = center[1] - df['y'][frame]
...
def min_by_min_top_bottom_analysis(df, tank_coordinates, trial, freeze_bin=0.5, 
    ...
    Parameters = ['top 1/2', 'bottom 1/2', 'top 1/3', 'middle 1/3', 'bottom 1/3', 
                  'distance from bottom', 'distance from center', 'freezing']    
    ...
    if mode.lower() == "time":
        ...
        for i in range(0,len(frame_index)-1):    
            ...
            for frame in frames:
                ...
                Output['distance from center'] += distance_from_center(df, frame, top, bottom, left, right)
    ...
    df_out.ix['distance from center'] = df_out.ix['distance from center'] / 100
    ...
    if use_real_dist:
        if real_len[0].lower() == "x":
            ...
            df_out.ix['distance from center'] = df_out.ix['distance from center'] * pix_con
